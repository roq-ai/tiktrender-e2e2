...
import { useStartupFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['digital-marketing-manager', 'content-creator', 'social-media-marketer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useStartupFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Startups',
      path: isTenantUser ? `/startups/view/${data?.id}` : '/startups',
      entity: 'startup',
      service: AccessServiceEnum.PROJECT,
      icon: FiBriefcase,
    },
    { name: 'Hashtags', path: '/hashtags', entity: 'hashtag', service: AccessServiceEnum.PROJECT, icon: FiHash },
    { name: 'Trends', path: '/trends', entity: 'trend', service: AccessServiceEnum.PROJECT, icon: FiTrendingUp },
