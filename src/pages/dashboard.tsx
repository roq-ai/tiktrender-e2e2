...
import { useRoqPlatformUserProfiles, useHashtagCount, useVideoTrendCount, useTrendCount } from 'lib/roq';

import { HashtagListPage as ShowCaseList } from 'pages/hashtags';

function DashboardPage() {
  const { session } = useSession();

  const hashtagStat = useHashtagCount();

  const videoTrendStat = useVideoTrendCount();

  const trendStat = useTrendCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Hashtags',
      result: hashtagStat?.data,
      isLoading: hashtagStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Video Trends',
      result: videoTrendStat?.data,
      isLoading: videoTrendStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Trends',
      result: trendStat?.data,
      isLoading: trendStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`hashtags`} passHref legacyBehavior>
                  <Button
