...
import { useHashtagFindFirst, useRoqClient } from 'lib/roq';

import { HashtagInterface } from 'interfaces/hashtag';

function HashtagViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const roqClient = useRoqClient();
  const queryParams = useMemo<{ include: { video: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useHashtagFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('hashtag', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/hashtags/edit/${id}`} passHref legacyBehavior>
                    <Button
