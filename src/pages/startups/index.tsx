...
import { useRoqClient, useStartupFindManyWithCount } from 'lib/roq';
...
export function StartupListPage(props: StartupListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useStartupFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.startup.delete({ where: { id } });
      await mutate();
    } catch (error) {
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/startups/view/${record.id}`} passHref legacyBehavior>
                <Button
