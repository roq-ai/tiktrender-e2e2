...
import { useRoqClient, useStartupFindFirst } from 'lib/roq';
...
function StartupEditPage() {
  ...
  const { data, error, isLoading, mutate } = useStartupFindFirst(queryParams, {}, { disabled: !id });
  ...
  const handleSubmit = async (values: StartupInterface, { resetForm }: FormikHelpers<any>) => {
    ...
    try {
      ...
      const updated = await roqClient.startup.update({
        data: values as RoqTypes.startup,
        where: {
          id,
        },
      });
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper wrapperProps={{ mb: 3 }}>
          <ImagePicker ref={imageUploaderRef} onChange={handleImageChange} entity="startup" entityId={id} />
        </FormWrapper>
