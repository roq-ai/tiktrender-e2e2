...
import { useRoqClient, useVideoFindManyWithCount } from 'lib/roq';
...
export function VideoListPage(props: VideoListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'hashtag.count', 'video_trend.count'],
  ...
  const { data, error, isLoading, mutate } = useVideoFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.video.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('hashtag', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'hashtag',
          header: 'Hashtag',
          accessorKey: 'hashtag',
          cell: ({ row: { original: record } }: any) => record?._count?.hashtag,
        }
      : null,
