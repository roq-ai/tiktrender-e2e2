...
import { useRoqClient, useVideoFindFirst } from 'lib/roq';
...
function VideoEditPage() {
  ...
  const { data, error, isLoading, mutate } = useVideoFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: VideoInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.video.update({
        data: values as RoqTypes.video,
        where: {
          id,
        },
      });
